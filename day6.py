...
def process_command(grid, command):
    start_x, start_y = command.start
    end_x, end_y = command.end

    if start_x == end_x:
        x_iterator = [start_x]
    else:
        x_iterator = range(start_x, end_x + 1)

    if start_y == end_y:
        y_iterator = [start_y]
    else:
        y_iterator = range(start_y, end_y + 1)

    for xx, yy in product(x_iterator, y_iterator):
        grid[xx][yy] = command.action(grid[xx][yy])
...
if __name__ == '__main__':
    ...
    xmas_grid = generate_grid(1000, 1000)
